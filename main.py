...
train_transform = transforms.Compose(  # 对训练图片进行预处理变换
    [
        ...
        transforms.RandomResizedCrop(224),  # 将图片尺寸裁切为224*224
        ...
        if self.mode == "semi":
            ...
            for i in tqdm(range(5)):  # tqdm使过程形象化
                ...
                if i == 0:
                    ...
                    X = np.concatenate((X, xi), axis=0)  # 在第0维进行数组拼接
...
def train_val(model, train_loader, val_loader, no_label_loader,
              device, epochs, opeimizer, loss, thres, save_path):
    model = model.to(device)
    semi_loader = None
    plt_train_loss = []
    plt_val_loss = []
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            pred = model(x)
            train_bat_loss = loss(pred, target)  # 由超参定义的loss
            train_bat_loss.backward()
            opeimizer.step()  # 起到更新模型的作用——Adamw
            opeimizer.zero_grad()  # 梯度清零，避免累计
        ...
        if epoch % 3 == 0 and plt_val_acc[-1] > 0.7:  # 每隔三轮进行一次半监督检查
